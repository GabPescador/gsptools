#' Imports diGly TMT data from fragpipe and normalizes with NormalyzerDE
#'
#' This function takes an inputPath with "metadata.csv", "contrast.csv" and
#' "abundance_single-site_MD.tsv", performs normalization with NormalyzerDE and saves
#' all outputs in the ouputPath. Examples of input files can be found in inst/extdata/importFragpipeTMT_examples.
#'
#' @param inputPath Input path where "metadata.csv", "contrast.csv" and "abundance_single-site.tsv" are located.
...
importFragpipeTMTdiGly <- function(inputPath, jobname, outputPath, force = FALSE){
  ...
  if (file.exists(paste0(inputPath, "metadata.csv")) &
    ...
    stop("No metadata.csv, contrasts.csv and/or abundance_single-site_MD.tsv found, stopping execution.")
  ...
  # Read input file and remove contaminants and rev proteins
      tmt <- fread(here(inputPath, "abundance_single-site_MD.tsv")) %>%
             rename("ModifiedSite" = "Index",
                    "ProteinName" = "Gene",
                    "ProteinID" = "ProteinID") %>%
  ...
  ### Matrix table
      matrix <- tmt %>%
        select(modified_site, reference_intensity:last_col(), -reference_intensity) %>%
        as.data.frame() %>%
        column_to_rownames("modified_site") %>%
        as.matrix()
